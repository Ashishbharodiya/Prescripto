...
function Doctor() {
  ...
  const getDashData = async () => {
    try {
      const response = await axios.get('https://prescripto-3-ry9r.onrender.com/api/doctor/dashboard', Dtoken);
  ...
  const getAppointments = async () => {
    try {
      const response = await axios.get('https://prescripto-3-ry9r.onrender.com/api/doctor/appointments', Dtoken);
      if (response.data.success) {
  ...
  const completeAppointment = async (appointmentId) => {
    try {
      const response = await axios.post(
        'https://prescripto-3-ry9r.onrender.com/api/doctor/complete-appointment',
  ...
  const cancelAppointment = async (appointmentId) => {
    try {
      const response = await axios.post(
        'https://prescripto-3-ry9r.onrender.com/api/doctor/cancel-appointment',
  ...
  return dashData && (
    <>
      <div className='bg-gray-900 dark:bg-gray-800 w-full h-full'>
        <p className='p-4 text-center text-lg font-medium text-white bg-gray-900 dark:bg-gray-800'>Doctor Dashboard</p>
          ...
          <div className='pt-4 border border-t-0 border-gray-800 dark:border-gray-700'>
            {dashData.latestAppointments.slice(0, 5).map((item, index) => (
              <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-800 dark:hover:bg-gray-700' key={index}>
                <img className='rounded-full w-10' src={`https://prescripto-3-ry9r.onrender.com${item.docData.image}`} alt="" />
